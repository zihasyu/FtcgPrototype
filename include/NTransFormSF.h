...
typedef vector<feature_t> NTransSuperFeatures;
...
class NTransIndexTable
{
public:
    NTransIndexTable() {};
    ~NTransIndexTable() {};

    // get the super features of the value
    ...
    unordered_map<feature_t, set<string>> original_feature_key_table_;
    map<string, feature_t> orignal_key_feature_table_;

    map<string, NTransSuperFeatures> key_feature_table_;

    unordered_map<feature_t, unordered_set<string>> feature_key_table_;
    // map<string, vector<uint64_t>> key_feature_table_;

private:
    void ExecuteDelete(const string &key, const NTransSuperFeatures &super_features);
    bool GetSuperFeatures(const string &key, NTransSuperFeatures *super_features);
};
#endif