...
void bruteforce::ProcessOneTrace()
{
    ...
    vector<bool> isMerged(EXPERIMENT_CHUNK_NUM, false);
    vector<vector<int64_t>> mergedReduceSizeTable(EXPERIMENT_CHUNK_NUM, vector<int64_t>(EXPERIMENT_CHUNK_NUM, 0));
    int64_t bestMergeReduceSize = 0;
    pair<int, int> bestMergePair;
    map<int, int64_t> mergedGroupSize;
    map<int, set<int>> mergedGroup;
    for (int i = 0; i < EXPERIMENT_CHUNK_NUM; i++)
    {
    ...
    for (int i = 0; i < EXPERIMENT_CHUNK_NUM; i++)
    {
        ...
        memcpy(clusterBuffer, chunkSet[i].chunkContent, chunkSet[i].chunkSize);
        for (int j = i + 1; j < EXPERIMENT_CHUNK_NUM; j++)
        {
            if (isMerged[j])
            {
                continue;
