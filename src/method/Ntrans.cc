...
NtransFeatureGen::NtransFeatureGen()
{
...
int NtransFeatureGen::getFeatureList(u_char *chunkBufferIn, int chunkSize, vector<uint64_t> &featureList, vector<uint64_t> &sf)
{
    for (int i = 0; i < FEATURE_NUMBER; ++i)
        feature[i] = 0;
...
int NtransFeatureGen::querySF(const vector<uint64_t> &sf)
{
    for (int i = 0; i < SF_NUMBER; ++i)
    {
...
void NtransFeatureGen::insertSFIndex(vector<uint64_t> &sf, uint64_t id)
{
    for (int i = 0; i < SF_NUMBER; ++i)
    {
...
void NtransFeatureGen::insert(int label)
{
    for (int i = 0; i < SF_NUMBER; ++i)
    {
...
void NTransIndexTable::Delete(const string &key)
{
    NTransSuperFeatures super_features;
    if (GetSuperFeatures(key, &super_features))
    {
        ExecuteDelete(key, super_features);
    }
}
void NTransIndexTable::ExecuteDelete(const string &key,
                                     const NTransSuperFeatures &super_features)
{
    for (const feature_t &sf : super_features)
    {
...
void NTransIndexTable::Put(const string &key, const string &value)
{
    vector<uint64_t> super_features(3);
    vector<feature_t> feature_list;
    // delete old feature if it exits so we can insert a new one
    Delete(key);
    ntransFeatureGen.getFeatureList(reinterpret_cast<u_char *>(const_cast<char *>(value.c_str())), CHUNK_SIZE, feature_list, super_features);
    vector<NTransSuperFeatures> superFeatures;
    key_feature_table_[key] = super_features;
    feature_key_table_[super_features[0]].insert(key);
}

bool NTransIndexTable::GetSuperFeatures(const string &key, NTransSuperFeatures *super_features)
{
